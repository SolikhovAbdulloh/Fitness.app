...
import {
  EXERCISE_ID_MAPPING,
  exerciseCounterLoader,
} from "../utils/motionDetectLogic/exerciseCounterLoader.js";
...
export const RoutineSession = ({ title = "Routine Session" }) => {
  ...
  // Receive from the StartRoutineModal as a prop
  ...
  useEffect(() => {
    ...
    const fetchRoutineInfo = async () => {
      try {
        ...
        console.log("RoutineSession", exercises);
  ...
  //   console.log("🏋️‍♂️ Exercise counter yuklanmoqda...");
  //   console.log("Selected exercise:", selectedExercise);

  //   if (selectedExercise) {
  //     console.log("Exercise ID:", selectedExercise.exercise_id);
  //     console.log("Available counters:", Object.keys(exerciseCounterLoader));
  ...
  //       console.log("✅ Counter yaratildi:", counter);
  //       return counter;
  //     } else {
  //       console.error("❌ Exercise counter topilmadi:", selectedExercise.name);
  //       console.log("Mavjud ID'lar:", Object.keys(exerciseCounterLoader));
  //       return null;
  //     }
  //   } else {
  //     console.log("❌ selectedExercise null");
  //     return null;
  //   }
  // };
  const loadExerciseCounter = (selectedExercise) => {
  ...
  // RoutineSession.jsx da predictPosture funksiyasiga debug qo'shing

  // Pozitsiya aniqlash va harakat sanash
                ...
                if (!calorieUpdatedRef.current) {
                  setCalorie((prevCalorie) => {
                    ...
                    const weight = userInfo.weight
                      ...
                      : 60;
                    return (
                      Math.round((prevCalorie + calorie * weight) * 10) / 10
                    );
                  });
  ...
  const incrementSetsCount = useCallback(() => {
    setSuccessSetCount((prevSuccessSetCount) => {
      const newSetCount = prevSuccessSetCount + 1;
      if (newSetCount < routine[selectedExerciseIndex].sets) {
